 ...
 * servicio/fecha/hora/nombre/email/teléfono/personas/notas del mensaje para
 * modificaciones conversacionales de reservas existentes.
 *
 ...
 *   un update validado (ver sanitizeBookingEntities() en chat-core.js, que reutiliza
...
// Los campos que el intérprete puede transcribir para una modificación.
// tablePreference/
...
// Extrae un bloque JSON válido de texto crudo devuelto por la IA.
// Limpia code fences (```json ... ```) y texto prosaico alrededor.
export function extractJsonFromText(rawText) {
  ...
  try {
    ...
    // Continúa a limpia de markdown y búsqueda de llaves
  }

  // 1. Limpieza de markdown code fences ```json ... ``` o ``` ... ```
  const fenceMatch = text.match(/```(?:json)?\s*([\s\S]*?)\s*```/i);
  if (fenceMatch && fenceMatch[1]) {
    try {
      return JSON.parse(fenceMatch[1].trim());
    } catch (e) {
      // Continúa a búsqueda por delimitadores {}
    }
  }

  // 2. Búsqueda de delimitadores de objeto JSON externo: de primer '{' a último '}'
