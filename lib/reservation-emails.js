...
const L = {
  es: {
    ...
    ownerCancelReservation: 'Cancelar esta reserva',
    phone: 'Teléfono:',
    email: 'Email:',
  },
  en: {
    ...
    ownerCancelReservation: 'Cancel this reservation',
    phone: 'Phone:',
    email: 'Email:',
  },
};
...
// Link de cancelación para el DUEÑO: página estática mínima (sin chat ni IA)
// que cancela con el mismo actionToken de un solo uso, verificado server-side
// con hash + expiración. El token va en el fragmento de la URL, nunca en query
// string, para que no quede en logs del servidor (mismo criterio que el enlace
// del cliente). [NUEVA FUNCIÓN — cancelación del dueño desde el correo]
export function ownerCancelReservationUrl(client, reservation) {
  return `${APP_URL}/cancelar-reserva/${encodeURIComponent(client.id || reservation.clientId)}#token=${encodeURIComponent(reservation.actionToken)}`;
}
...
export async function sendReservationEmails(client, reservation, type, deps) {
  ...
  const phoneLine = reservation.telefono ? `<p style="margin:0 0 8px;font-size:14px;color:#495057"><strong>${lo.phone}</strong> ${esc(reservation.telefono)}</p>` : '';
  const emailLine = reservation.email ? `<p style="margin:0 0 8px;font-size:14px;color:#495057"><strong>${lo.email}</strong> ${esc(reservation.email)}</p>` : '';

  const ownerContactBox = `
    <div style="background:#f8f9fa;border:1px solid #e9ecef;border-radius:12px;padding:16px;margin:20px 0">
      ${phoneLine}${emailLine}<p style="margin:0;font-size:14px;color:#495057"><strong>${lo.specialRequests}</strong> ${esc(reservation.specialRequests || lo.noSpecialRequests)}</p>
    </div>
  `;
  ...
  // Botón de cancelación para el DUEÑO: solo en avisos de reserva activa (no en
  // el correo de "fue cancelada") y solo si el token de un solo uso existe.
  // Mismo criterio que el botón del cliente en reservationEmailHtml. Apunta a
  // la página estática /cancelar-reserva/:id, sin pasar por el chat ni la IA.
  const ownerCancelBtn = (type !== 'cancelled' && reservation.actionToken)
    ? `<p style="margin:20px 0 0;text-align:center">
        <a href="${esc(ownerCancelReservationUrl(client, reservation))}" style="display:inline-block;background:#b23b3b;color:#ffffff;text-decoration:none;padding:12px 24px;border-radius:8px;font-weight:600;font-size:14px">${lo.ownerCancelReservation}</a>
      </p>`
    : '';

  const ownerHtml = shell(`<p style="font-size:15px;line-height:1.6;color:#212529">${ownerIntroText}</p>${ownerContactBox}${ownerCancelBtn}${ownerDashboardBtn}`,
...
export async function sendBillingAlertEmail(client, type, extra = {}) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) return { attempted: false, error: 'RESEND_API_KEY missing' };
  const resend = new Resend(apiKey);

  const recipients = destinatariosAviso(client);
  if (!recipients.length) return { attempted: false, error: 'No recipient emails' };

  const color = client.color || '#1a4a2e';
  const businessName = client.businessName || 'tu negocio';
  const clientId = client.id || extra.clientId || '';

  const portalUrl = client.panelToken
    ? `${APP_URL}/reservas/${encodeURIComponent(clientId)}#t=${encodeURIComponent(client.panelToken)}`
    : `${APP_URL}/reservas/${encodeURIComponent(clientId)}`;
  ...
  if (type === 'payment_failed') {
    subject = `Aviso de pago: Tu suscripción en JB Studio`;
    title = `Aviso sobre tu pago`;
    const attemptDateStr = extra.gracePeriodEndsAt
      ? `el <strong>${esc(extra.gracePeriodEndsAt)}</strong>`
      : 'próximamente';
    bodyText = `
      <p style="font-size:15px;line-height:1.6;color:#212529">
        Hola. No pudimos procesar el cobro de tu suscripción para <strong>${esc(businessName)}</strong>.
      </p>
      <p style="font-size:15px;line-height:1.6;color:#212529">
        Vamos a reintentarlo automáticamente ${attemptDateStr}. Tu servicio y asistente virtual continúan activos por ahora.
      </p>
      <p style="font-size:15px;line-height:1.6;color:#212529">
        Si deseas actualizar tu tarjeta o método de pago ahora mismo, puedes hacerlo desde tu panel:
      </p>
    `;
  } else if (type === 'subscription_paused') {
    subject = `Suscripción pausada: JB Studio`;
    title = `Suscripción pausada`;
    bodyText = `
      <p style="font-size:15px;line-height:1.6;color:#212529">
        Hola. Tu suscripción para <strong>${esc(businessName)}</strong> ha sido pausada por falta de pago y tu chatbot ha sido desactivado temporalmente.
      </p>
      <p style="font-size:15px;line-height:1.6;color:#212529">
        Actualiza tu tarjeta o método de pago para reactivar tu chatbot y restaurar el servicio de inmediato:
      </p>
    `;
  } else if (type === 'trial_ending_soon') {
    subject = `Tu prueba gratuita termina pronto: JB Studio`;
    title = `Tu prueba gratuita termina pronto`;
    const trialEnd = extra.trialEnd
      ? `el <strong>${esc(new Date(extra.trialEnd).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' }))}</strong>`
      : 'próximamente';
    bodyText = `
      <p style="font-size:15px;line-height:1.6;color:#212529">
        Hola. La prueba gratuita de <strong>${esc(businessName)}</strong> termina ${trialEnd}.
      </p>
      <p style="font-size:15px;line-height:1.6;color:#212529">
        Se cobrará automáticamente el plan seleccionado al finalizar la prueba, salvo que canceles antes.
      </p>
      <p style="font-size:15px;line-height:1.6;color:#212529">
        Puedes revisar o cancelar tu suscripción desde tu panel:
      </p>
    `;
  } else {
    return { attempted: false, error: 'Unknown email type' };
  }

  const btnHtml = `
    <p style="margin:24px 0 8px;text-align:center">
      <a href="${esc(portalUrl)}" style="display:inline-block;background:${esc(color)};color:#ffffff;text-decoration:none;padding:12px 24px;border-radius:8px;font-weight:600;font-size:14px">Gestionar suscripción</a>
    </p>
  `;

  const html = shell(`${bodyText}${btnHtml}`, title, color, businessName, client.language || 'es');

  const result = { attempted: true, recipients, sent: false, error: null, messageIds: [] };
  for (const to of recipients) {
    try {
      const r = await resend.emails.send({ from: FROM, to, subject, html });
      if (r && r.error) {
        result.error = r.error.message || 'send failed';
        console.error(`[reservation-emails] billing alert failed: type=${type} clientId=${clientId} error=${result.error}`);
      } else {
        result.sent = true;
        const id = resendMessageId(r);
        if (id) result.messageIds.push(id);
        console.log(`[reservation-emails] billing alert sent: type=${type} clientId=${clientId} messageId=${id || 'unknown'}`);
      }
    } catch (e) {
      result.error = e.message || 'send threw';
      console.error(`[reservation-emails] billing alert threw: type=${type} clientId=${clientId} error=${result.error}`);
    }
  }
  return result;
}
