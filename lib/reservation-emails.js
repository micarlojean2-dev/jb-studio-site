...
export function reservationEmailHtml(client, reservation, type) {
  ...
  const actions = type === 'cancelled' ? '' : `<p style="margin:20px 0 0"><a href="${esc(reservationActionUrl(reservation, 'cancel'))}" style="display:inline-block;background:#b23b3b;color:#fff;text-decoration:none;padding:11px 16px;border-radius:8px;font-weight:600">${l.cancel}</a> <a href="${esc(reservationActionUrl(reservation, 'reschedule'))}" style="display:inline-block;background:${esc(client.color || '#1a4a2e')};color:#fff;text-decoration:none;padding:11px 16px;border-radius:8px;font-weight:600">${l.reschedule}</a></p>`;
...
export async function sendReservationEmails(client, reservation, type, deps) {
  ...
  const ownerSubject = `${client.businessName} - ${L[ownerLang].subject[titleKey]}`;
  const details = reservationEmailHtml(client, reservation, type);

  const lo = L[ownerLang];
  const ownerIntro = type === 'cancelled'
    ? lo.ownerCancelledIntro(reservation.nombre)
    : `<strong>${esc(reservation.nombre)}</strong>: ${esc(reservation.servicio || (reservation.partySize ? lo.people(reservation.partySize) : ''))}`;
  // Solo el dueño ve el contacto del cliente (teléfono/email); el email que
  // recibe el propio cliente (reservationEmailHtml, arriba) no cambia. Cada
  // línea es condicional: si el dato no existe, no deja una línea vacía.
  const ownerContact = [
    reservation.telefono ? `<p style="margin:4px 0 0"><strong>${lo.phone}</strong> ${esc(reservation.telefono)}</p>` : '',
    reservation.email ? `<p style="margin:4px 0 0"><strong>${lo.email}</strong> ${esc(reservation.email)}</p>` : '',
  ].join('');
  const ownerHtml = shell(`<p style="font-size:15px;line-height:1.55">${ownerIntro}</p>${ownerContact}<p>${esc(reservation.fecha)} · ${esc(reservation.hora)}</p><p><strong>${lo.specialRequests}</strong> ${esc(reservation.specialRequests || lo.noSpecialRequests)}</p>`,
