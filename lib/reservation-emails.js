...
export async function sendReservationEmails(client, reservation, type, deps) {
  ...
  const ownerContact = [
    reservation.telefono ? `<p style="margin:4px 0 0"><strong>Teléfono:</strong> ${esc(reservation.telefono)}</p>` : '',
    reservation.email ? `<p style="margin:4px 0 0"><strong>Email:</strong> ${esc(reservation.email)}</p>` : '',
  ].join('');
  const ownerHtml = shell(`<p style="font-size:15px;line-height:1.55">${ownerIntro}</p>${ownerContact}<p>${esc(reservation.fecha)} · ${esc(reservation.hora)}</p><p><strong>Peticiones especiales:</strong> ${esc(reservation.specialRequests || 'Sin peticiones especiales')}</p>`, subject, client.color || '#1a4a2e', client.businessName || 'Reserva');
