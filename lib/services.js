...
//
// Dos servicios con el mismo id romperían findServiceByLinkedItemId (un
// linkedItemId por id encontraría el primero que matchee, ambiguo). Si el
// array de entrada ya trae un id repetido (dato corrupto, copia manual,
// etc.), el primero conserva ese id y el resto se regenera.
export function sanitizeServiceList(rawList, limit) {
  ...
  return rawList.slice(0, limit).map(item => {
    ...
    if (seenIds.has(id)) id = createServiceId();
    seenIds.add(id);
    return {
      id,
